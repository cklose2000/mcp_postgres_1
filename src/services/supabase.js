...
export async function listTables() {
    const result = await executeSqlQuery("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'");
    if (result.error) {
...
export async function getTableSchema(tableName) {
    ...
    const result = await executeSqlQuery(`SELECT column_name, data_type FROM information_schema.columns WHERE table_name = '${sanitizedTableName}'`);
    if (result.error) {
